    ...
    renderItem(): React.JSX.Element {
        return (
            <div style={{ width: '100%', margin: '0 0 1rem 0' }}>
                <div style={{ width: '100%', margin: '0 0 1rem 0' }}>
                    <span hidden={!!this.state.dropboxUrl}>
                ...
                >
                    {I18n.t(this.props.data.dropboxAccessJson ? 'Renew Dropbox Access' : 'Get Dropbox Access')}
                </Button>
                            ...
                            onClick={() =>
                                this.setState({ running: true }, async () => {
                                    const result = await this.props.oContext.socket.sendTo(
                                        `${this.props.oContext.adapterName}.${this.props.oContext.instance}`,
                                        'authDropbox',
                                        { code: this.state.code, codeChallenge: this.props.data.dropboxCodeChallenge },
                                    );
                                    this.props.onChange({ ...this.props.data, dropboxAccessJson: result.json });
                                    this.setState({
                                        dropboxState: result.done ? 'Present' : 'Not present',
                                        running: false,
                                        dropboxUrl: null,
                                    });
                                })
