...
class Restore extends Component<RestoreProps, RestoreState> {
    ...
    constructor(props: RestoreProps) {
        super(props);
        const WITH_DOWNLOAD = ['dropbox', 'onedrive', 'googledrive', 'ftp', 'webdav'];
        const NO_RESTART = [
            'grafana',
            'jarvis',
            'javascripts',
        ...
        if (!NO_RESTART.find(text => this.props.fileName.includes(text))) {
            ...
            if (WITH_DOWNLOAD.includes(this.props.location)) {
                messages[0] = {
                    text: I18n.t(
                        'Confirm with "Restore" and the download begins. Please wait until the download is finished!',
                    ),
                    number: true,
                };
                messages[1] = {
                    text: I18n.t('After download ioBroker will be restarted during restore.'),
                    number: true,
                };
            } else {
            ...
            messages.push({
                text: I18n.t('If the tab does not open, please deactivate your popup blocker.'),
                number: false,
            });
        } else {
            ...
            await fetch(`${window.location.protocol}//${window.location.hostname}:8091/status.json`)
                ...
                .then(data => {
                    const restoreProcess: {
                        ...
                        restoreStatus: string;
                        statusColor: string;
                    } = JSON.parse(JSON.stringify(this.state.restoreProcess));
                    ...
                    // scroll down
                    if (
                        this.textRefRestore.current &&
                        this.textRefRestore.current.scrollTop + this.textRefRestore.current.clientHeight >=
                            this.textRefRestore.current.scrollHeight
                    ) {
                        setTimeout(
                            () =>
                                this.textRefRestore.current &&
                                (this.textRefRestore.current.scrollTop = this.textRefRestore.current.scrollHeight),
                            100,
                        );
                    }
    ...
    onOutput = (_id: string, state: ioBroker.State | null | undefined): void => {
        if (state?.val && state.val !== this.lastExecutionLine) {
    ...
    async componentDidMount(): Promise<void> {
        await this.props.socket.subscribeState(
            `${this.props.adapterName}.${this.props.instance}.output.line`,
            this.onOutput,
        );
    ...
    static renderLine(line: ExecutionLine, i: number, isFullScreen: boolean): React.JSX.Element {
        return (
            <div
                key={i}
                style={{ ...(isFullScreen ? styles.responseTextLine : styles.textLine) }}
            >
                <div style={{ ...styles.textTime, ...(line.level ? styles[`textLevel-${line.level}`] || {} : {}) }}>
                    {line.ts}
                </div>
                <div style={{ ...styles.textLevel, ...(line.level ? styles[`textLevel-${line.level}`] || {} : {}) }}>
                    {line.level}
                </div>
                <div style={{ ...styles.textSource, ...(line.level ? styles[`textLevel-${line.level}`] || {} : {}) }}>
                    {line.source}
                </div>
                <div
                    style={{
                        ...(isFullScreen ? styles.responseText : styles.text),
                        ...(line.level ? styles[`textLevel-${line.level}`] || {} : {}),
                    }}
                >
                    {line.text}
                </div>
    ...
    static renderRestoreLine(line: string, i: number, isFullScreen: boolean): React.JSX.Element {
        return (
            <div
                key={i}
                style={{ ...(isFullScreen ? styles.responseTextLine : styles.textLine) }}
    ...
    renderRestoreDialog(): React.JSX.Element | null {
        if (!this.state.showRestoreDialog) {
            return null;
        }
        return (
            <Dialog
                open={!0}
                onClose={() => {
                    this.state.restoreProcess.done && this.setState({ showRestoreDialog: false });
                }}
                maxWidth="lg"
                fullWidth
            ...
            >
                ...
                <DialogContent style={styles.dialogContent}>
                    ...
                    >
                        {this.state.restoreProcess.log.map((line, i) =>
                            Restore.renderRestoreLine(line, i, this.state.isFullScreen),
                        )}
                    </div>
                </DialogContent>
        ...
        return (
            ...
            >
                ...
                <DialogContent style={styles.dialogContent}>
                    ...
                    {this.state.executing || this.state.done ? (
                        <div
                            style={{
                                ...styles.logContainer,
                                ...(this.state.isFullScreen ? styles.responseLogContainer : undefined),
                            ...
                            ref={this.textRef}
                        >
                            {this.state.executionLog.map((line, i) =>
                                Restore.renderLine(line, i, this.state.isFullScreen),
                            )}
                        </div>
                    ) : null}
                    {!this.state.executing && !this.state.done ? (
