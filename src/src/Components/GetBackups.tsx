...
import { Close, Download, History, ExpandMore, Save, FormatListBulleted, SolarPower } from '@mui/icons-material';
...
function getIcon(type: string): React.JSX.Element | null {
    if (!ICONS[type]) {
        return null;
    }
...
function getLabelByValue(value: string, connectType: string): string {
    const STORAGENAME = [
...
interface GetBackupsProps {
    ...
    themeType: ThemeType;
    onClose: () => void;
    onRestore: (location: string, object: string, path: string) => void;
...
export default function GetBackups(props: GetBackupsProps): React.JSX.Element {
    const fullScreen = useMediaQuery(props.themeBreakpoints('sm'));
    const [backups, setBackups] = useState<null | {
        data: Record<string, Record<string, { name: string; size: number; path: string }[]>>;
    }>(null);
        ...
        void props.socket
            ...
            .then(
                (result: {
                    ...
                    Object.values(result.data).forEach(location =>
