...
.ace_snippet-marker {
    ...
    position: absolute;
}`,"snippets.css",!1),n.snippetManager=new z;var y=T("./editor").Editor;(function(){this.insertSnippet=function(p,g){return n.snippetManager.insertSnippet(this,p,g)},this.expandSnippet=function(p){return n.snippetManager.expandWithTab(this,p)}}).call(y.prototype)}),ace.define("ace/autocomplete/popup",["require","exports","module","ace/virtual_renderer","ace/editor","ace/range","ace/lib/event","ace/lib/lang","ace/lib/dom","ace/config","ace/lib/useragent"],function(T,n,C){"use strict";var m=T("../virtual_renderer").VirtualRenderer,D=T("../editor").Editor,v=T("../range").Range,i=T("../lib/event"),u=T("../lib/lang"),a=T("../lib/dom"),_=T("../config").nls,P=T("./../lib/useragent"),f=function(R){return"suggest-aria-id:".concat(R)},Z=P.isSafari?"menu":"listbox",j=P.isSafari?"menuitem":"option",z=P.isSafari?"aria-current":"aria-selected",ie=function(R){var w=new m(R);w.$maxLines=4;var y=new D(w);return y.setHighlightActiveLine(!1),y.setShowPrintMargin(!1),y.renderer.setShowGutter(!1),y.renderer.setHighlightGutterLine(!1),y.$mouseHandler.$focusTimeout=0,y.$highlightTagPending=!0,y},ue=function(){function R(w){var y=a.createElement("div"),p=ie(y);w&&w.appendChild(y),y.style.display="none",p.renderer.content.style.cursor="default",p.renderer.setStyle("ace_autocomplete"),p.renderer.$textLayer.element.setAttribute("role",Z),p.renderer.$textLayer.element.setAttribute("aria-roledescription",_("autocomplete.popup.aria-roledescription","Autocomplete suggestions")),p.renderer.$textLayer.element.setAttribute("aria-label",_("autocomplete.popup.aria-label","Autocomplete suggestions")),p.renderer.textarea.setAttribute("aria-hidden","true"),p.setOption("displayIndentGuides",!1),p.setOption("dragDelay",150);var g=function(){};p.focus=g,p.$isFocused=!0,p.renderer.$cursorLayer.restartTimer=g,p.renderer.$cursorLayer.element.style.opacity="0",p.renderer.$maxLines=8,p.renderer.$keepTextAreaAtCursor=!1,p.setHighlightActiveLine(!1),p.session.highlight(""),p.session.$searchHighlight.clazz="ace_highlight-marker",p.on("mousedown",function(S){var I=S.getDocumentPosition();p.selection.moveToPosition(I),b.start.row=b.end.row=I.row,S.stop()});var d,x=new v(-1,0,-1,1/0),b=new v(-1,0,-1,1/0);b.id=p.session.addMarker(b,"ace_active-line","fullLine"),p.setSelectOnHover=function(S){S?x.id&&(p.session.removeMarker(x.id),x.id=null):x.id=p.session.addMarker(x,"ace_line-hover","fullLine")},p.setSelectOnHover(!1),p.on("mousemove",function(S){if(!d){d=S;return}if(!(d.x==S.x&&d.y==S.y)){d=S,d.scrollTop=p.renderer.scrollTop,p.isMouseOver=!0;var I=d.getDocumentPosition().row;x.start.row!=I&&(x.id||p.setRow(I),$(I))}}),p.renderer.on("beforeRender",function(){if(d&&x.start.row!=-1){d.$pos=null;var S=d.getDocumentPosition().row;x.id||p.setRow(S),$(S,!0)}}),p.renderer.on("afterRender",function(){for(var S=p.renderer.$textLayer,I=S.config.firstRow,U=S.config.lastRow;I<=U;I++){var oe=S.element.childNodes[I-S.config.firstRow];oe.setAttribute("role",j),oe.setAttribute("aria-roledescription",_("autocomplete.popup.item.aria-roledescription","item")),oe.setAttribute("aria-setsize",p.data.length),oe.setAttribute("aria-describedby","doc-tooltip"),oe.setAttribute("aria-posinset",I+1);var K=p.getData(I);if(K){var ae="".concat(K.caption||K.value).concat(K.meta?", ".concat(K.meta):"");oe.setAttribute("aria-label",ae)}var he=oe.querySelectorAll(".ace_completion-highlight");he.forEach(function(V){V.setAttribute("role","mark")})}}),p.renderer.on("afterRender",function(){var S=p.getRow(),I=p.renderer.$textLayer,U=I.element.childNodes[S-I.config.firstRow],oe=document.activeElement;if(U!==p.selectedNode&&p.selectedNode&&(a.removeCssClass(p.selectedNode,"ace_selected"),p.selectedNode.removeAttribute(z),p.selectedNode.removeAttribute("id")),oe.removeAttribute("aria-activedescendant"),p.selectedNode=U,U){var K=f(S);a.addCssClass(U,"ace_selected"),U.id=K,I.element.setAttribute("aria-activedescendant",K),oe.setAttribute("aria-activedescendant",K),U.setAttribute(z,"true")}});var B=function(){$(-1)},$=function(S,I){S!==x.start.row&&(x.start.row=x.end.row=S,I||p.session._emit("changeBackMarker"),p._emit("changeHoverMarker"))};p.getHoveredRow=function(){return x.start.row},i.addListener(p.container,"mouseout",function(){p.isMouseOver=!1,B()}),p.on("hide",B),p.on("changeSelection",B),p.session.doc.getLength=function(){return p.data.length},p.session.doc.getLine=function(S){var I=p.data[S];return typeof I=="string"?I:I&&I.value||""};var k=p.session.bgTokenizer;return k.$tokenizeRow=function(S){function I(Ce,ke){Ce&&oe.push({type:(U.className||"")+(ke||""),value:Ce})}var U=p.data[S],oe=[];if(!U)return oe;typeof U=="string"&&(U={value:U});for(var K=U.caption||U.value||U.name,ae=K.toLowerCase(),he=(p.filterText||"").toLowerCase(),V=0,G=0,H=0;H<=he.length;H++)if(H!=G&&(U.matchMask&1<<H||H==he.length)){var X=he.slice(G,H);G=H;var Ve=ae.indexOf(X,V);if(Ve==-1)continue;I(K.slice(V,Ve),""),V=Ve+X.length,I(K.slice(Ve,V),"completion-highlight")}return I(K.slice(V,K.length),""),oe.push({type:"completion-spacer",value:" "}),U.meta&&oe.push({type:"completion-meta",value:U.meta}),U.message&&oe.push({type:"completion-message",value:U.message}),oe},k.$updateOnChange=g,k.start=g,p.session.$computeWidth=function(){return this.screenWidth=0},p.isOpen=!1,p.isTopdown=!1,p.autoSelect=!0,p.filterText="",p.isMouseOver=!1,p.data=[],p.setData=function(S,I){p.filterText=I||"",p.setValue(u.stringRepeat(`
`,S.length),-1),p.data=S||[],p.setRow(0)},p.getData=function(S){return p.data[S]},p.getRow=function(){return b.start.row},p.setRow=function(S){S=Math.max(this.autoSelect?0:-1,Math.min(this.data.length-1,S)),b.start.row!=S&&(p.selection.clearSelection(),b.start.row=b.end.row=S||0,p.session._emit("changeBackMarker"),p.moveCursorTo(S||0,0),p.isOpen&&p._signal("select"))},p.on("changeSelection",function(){p.isOpen&&p.setRow(p.selection.lead.row),p.renderer.scrollCursorIntoView()}),p.hide=function(){this.container.style.display="none",p.anchorPos=null,p.anchor=null,p.isOpen&&(p.isOpen=!1,this._signal("hide"))},p.tryShow=function(S,I,U,oe){if(!oe&&p.isOpen&&p.anchorPos&&p.anchor&&p.anchorPos.top===S.top&&p.anchorPos.left===S.left&&p.anchor===U)return!0;var K=this.container,ae=window.innerHeight,he=window.innerWidth,V=this.renderer,G=V.$maxLines*I*1.4,H={top:0,bottom:0,left:0},X=ae-S.top-3*this.$borderSize-I,Ve=S.top-3*this.$borderSize;U||(Ve<=X||X>=G?U="bottom":U="top"),U==="top"?(H.bottom=S.top-this.$borderSize,H.top=H.bottom-G):U==="bottom"&&(H.top=S.top+I+this.$borderSize,H.bottom=H.top+G);var Ce=H.top>=0&&H.bottom<=ae;if(!oe&&!Ce)return!1;Ce?V.$maxPixelHeight=null:U==="top"?V.$maxPixelHeight=Ve:V.$maxPixelHeight=X,U==="top"?(K.style.top="",K.style.bottom=ae-H.bottom+"px",p.isTopdown=!1):(K.style.top=H.top+"px",K.style.bottom="",p.isTopdown=!0),K.style.display="";var ke=S.left;return ke+K.offsetWidth>he&&(ke=he-K.offsetWidth),K.style.left=ke+"px",K.style.right="",p.isOpen||(p.isOpen=!0,this._signal("show"),d=null),p.anchorPos=S,p.anchor=U,!0},p.show=function(S,I,U){this.tryShow(S,I,U?"bottom":void 0,!0)},p.goTo=function(S){var I=this.getRow(),U=this.session.getLength()-1;switch(S){case"up":I=I<=0?U:I-1;break;case"down":I=I>=U?-1:I+1;break;case"start":I=0;break;case"end":I=U}this.setRow(I)},p.getTextLeftOffset=function(){return this.$borderSize+this.renderer.$padding+this.$imageSize},p.$imageSize=0,p.$borderSize=1,p}return R}();a.importCssString(`
