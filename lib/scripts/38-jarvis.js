...
async function saveState(options, file, tmpDir, log) {
    return new Promise(async (resolve) => {
        ...
        for (const i in _states) {
            try {
                ...
                if (obj) {
                    ...
                    log.warn(`settings "${_states[i]}" not found`);
                }
            } catch (err) {
                log.warn(`No State found for "jarvis.${file}.${_states[i]}": ${err}`);
            }
        }
        ...
        jarvisStates && await fs_async.writeFile(path.join(stateDir, `states.json`), JSON.stringify(jarvisStates, null, 2)).catch(err => log.warn(`states.json cannot be written: ${err}`));
        resolve();
    });
}
