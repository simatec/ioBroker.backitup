    ...
    return new Promise(async (resolve) => {
        ...
        if (_settings) {
            for (const i in _settings) {
                const obj = await options.adapter.getForeignStateAsync(`${_settings[i]._id}`).catch(() => log.debug(`No State found for "${_settings[i]._id}"`));
        ...
        for (const i in _states) {
            const obj = await options.adapter.getForeignStateAsync(`jarvis.${file}.${_states[i]}`).catch(() => log.debug(`No State found for "jarvis.${file}.${_states[i]}"`));
        ...
        const _pro = await options.adapter.getForeignStateAsync(`jarvis.${file}.info.pro`).catch(() => log.debug(`No State found for "jarvis.${file}.info.pro"`));
