...
function command(options, log, callback) {
    ...
    if (options.mountType === 'CIFS') {
        setTimeout(function () {
            child_process.exec(`${options.sudo ? 'sudo mount' : 'mount'} -t cifs -o ${options.user ? 'username=' + options.user + ',password=' + options.pass : ''}${options.cifsDomain ? ',domain=' + options.cifsDomain : ''},noserverino,rw,file_mode=0777,dir_mode=0777,${options.smb} ${options.mount}${options.dir} ${options.backupDir}`, (error, stdout, stderr) => {
                if (error) {
                    //let errLog = '' + error;
                    ...
                    child_process.exec(`${options.sudo ? 'sudo mount' : 'mount'} -t cifs -o ${options.user ? 'username=' + options.user + ',password=' + options.pass : ''}${options.cifsDomain ? ',domain=' + options.cifsDomain : ''},noserverino,rw,file_mode=0777,dir_mode=0777 ${options.mount}${options.dir} ${options.backupDir}`, (error, stdout, stderr) => {
                        if (error) {
                            let errLog = '' + error;
                            errLog = errLog.replace(new RegExp(options.pass, 'g'), "****")
