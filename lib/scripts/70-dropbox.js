    ...
    if (!fileNames || !fileNames.length) {
        ...
        if (fs.existsSync(fileName)) {
            try {
                ...
                if ((Math.round(fileSize / (1024 * 1024) * 10) / 10) <= 100) {
                ...
                } else if ((Math.round(fileSize / (1024 * 1024) * 10) / 10) > 100) {
                    await dropbox.getUpload(dbx, fileName, dir, log);
...
async function command(options, log, callback) {
    ...
    // Token refresh
    if (options.dropboxTokenType == 'default') {
        db_accessToken = await dropbox.getToken(options.dropboxAccessJson, log).catch(err => log.warn('Dropbox Token: ' + err));
    } else if (options.dropboxTokenType == 'custom') {
        db_accessToken = options.accessToken !== undefined ? options.accessToken : '';
    }
