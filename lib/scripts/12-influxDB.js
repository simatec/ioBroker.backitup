...
async function startBackup(options, log, callback) {
	return new Promise(async (resolve) => {
		...
		let influxDBCMD = `${options.exe ? `"${options.exe}"` : 'influxd'} backup -portable -database ${options.dbName}${options.dbType == 'remote' ? ` -host ${options.host}:${options.port}` : ''} "${tmpDir}"`;
		...
		*/
		...
		child_process.exec(influxDBCMD, async (error, stdout, stderr) => {
			if (error) {
