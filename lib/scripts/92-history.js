...
function command(options, log, callback) {
    if (options.history.enabled &&
        ...
        options.adapter.getState('history.json', (err, state) => {
            ...
            if (options.ftp && options.ftp.enabled) {
                historyArrayJSON.unshift(`{"Time":"${timeStamp}", "Type":"${options.name}", "Store":"${tools._('FTP-Backup: Yes', options.history.systemLang)}", "Error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (options.cifs && options.cifs.enabled) {
                historyArrayJSON.unshift(`{"Time":"${timeStamp}", "Type":"${options.name}", "Store":"${tools._('CIFS-Mount: Yes', options.history.systemLang)}", "Error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (options.dropbox && options.dropbox.enabled) {
                historyArrayJSON.unshift(`{"Time":"${timeStamp}","Type":"${options.name}","Store":"${tools._('Dropbox: Yes', options.history.systemLang)}","Error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (options.googledrive && options.googledrive.enabled) {
                historyArrayJSON.unshift(`{"Time":"${timeStamp}","Type":"${options.name}","Store":"${tools._('Google Drive: Yes', options.history.systemLang)}","Error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (!doneSomething) {
                historyArrayJSON.unshift(`{"Time":"${timeStamp}","Type":"${options.name}","Store":"${tools._('Only stored locally', options.history.systemLang)}","Error":"${errorMessage}"}`);
            }
