...
function command(options, log, callback) {
    if (options.history.enabled &&
        ...
        options.adapter.getState('history.json', (err, state) => {
            ...
            if (options.ftp && options.ftp.enabled) {
                historyArrayJSON.unshift(`{"date":"${timeStamp}", "type":"${options.name}", "storage":"${tools._('FTP-Backup: Yes', options.history.systemLang)}", "error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (options.cifs && options.cifs.enabled) {
                historyArrayJSON.unshift(`{"date":"${timeStamp}", "type":"${options.name}", "storage":"${tools._('CIFS-Mount: Yes', options.history.systemLang)}", "error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (options.dropbox && options.dropbox.enabled) {
                historyArrayJSON.unshift(`{"date":"${timeStamp}","type":"${options.name}","storage":"${tools._('Dropbox: Yes', options.history.systemLang)}","error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (options.googledrive && options.googledrive.enabled) {
                historyArrayJSON.unshift(`{"date":"${timeStamp}","type":"${options.name}","storage":"${tools._('Google Drive: Yes', options.history.systemLang)}","error":"${errorMessage}"}`);
                doneSomething = true;
            }
            if (!doneSomething) {
                historyArrayJSON.unshift(`{"date":"${timeStamp}","type":"${options.name}","storage":"${tools._('Only stored locally', options.history.systemLang)}","error":"${errorMessage}"}`);
            }
