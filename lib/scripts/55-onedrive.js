...
async function command(options, log, callback) {
    ...
    const od_accessToken = await onedrive.getToken(options.onedriveAccessJson, log).catch(err => log.warn('Onedrive Token: ' + err));
    ...
    if (od_accessToken && options.context.fileNames.length) {
        ...
        copyFiles(od_accessToken, dir, fileNames, log, options.context.errors, err => {
            ...
            if (options.deleteOldBackup === true) {
                cleanFiles(od_accessToken, options, dir, options.context.types, options.deleteBackupAfter, log, options.context.errors, err => {
                    if (err) {
