        ...
        try {
            ...
            if (!errors.length) {
                ...
                if (options.cifs && options.cifs.enabled) {
                    historyArray.unshift(`<span class="backup-type-${options.name}">${timeStamp} - ${tools._('Type', options.historyHTML.systemLang)}: ${options.name} - ${tools._('NAS: Yes', options.historyHTML.systemLang)}</span>`);
                    doneSomething = true;
                }
                ...
                if (options.onedrive && options.onedrive.enabled) {
                    historyArray.unshift(`<span class="backup-type-${options.name}">${timeStamp} - ${tools._('Type', options.historyHTML.systemLang)}: ${options.name} - ${tools._('Onedrive: Yes', options.historyHTML.systemLang)}</span>`);
                    doneSomething = true;
                }
                if (!doneSomething) {
