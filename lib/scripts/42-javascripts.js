...
async function command(options, log, callback) {
    ...
    options.adapter.getForeignObjects('script.*', 'script', async (err, obj) => {
        err && log.error(err);
        if (obj) {
        ...
        if (fs.existsSync(tmpDir) && obj) {
            ...
            compress({
                ...
                try {
                    ...
                    log.debug(`Try deleting the Javascript tmp file: "${tmpDir}"`);
                    fse.removeSync(tmpDir);
                    if (!fs.existsSync(tmpDir)) {
                        log.debug(`Javascript tmp file "${tmpDir}" successfully deleted`);
                    }
                } catch (err) {
                    log.debug(`Javascript tmp file "${tmpDir}" cannot deleted ... ${err}`);
                }
                if (err) {
