...
function command(options, log, callback) {
	...
	log.debug(`exec: ${options.exe ? options.exe : 'influxd'} backup -portable -database ${options.dbName}${options.type == 'remote' ? ` -host ${options.host}:${options.port}` : ''} ${tmpDir}`);
	child_process.exec(`${options.exe ? options.exe : 'influxd'} backup -portable -database ${options.dbName}${options.type == 'remote' ? ` -host ${options.host}:${options.port}` : ''} ${tmpDir}`, (error, stdout, stderr) => {
		if (error) {
			if (fs.existsSync(tmpDir)) {
				try {
