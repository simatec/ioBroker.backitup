    ...
    if (!fileNames || !fileNames.length) {
        ...
        if (fs.existsSync(fileName)) {
            try {
                ...
                //const writeStream = fs.createWriteStream(uploadLink);

                log.debug('WebDAV: Copy ' + onlyFileName + '...');
                //let fileContent = fs.readFileSync(fileName);

                // Upload File
                //await client.putFileContents(webdavFilename, fs.readFileSync(fileName), { format: "binary", "Content-Type": "application/octet-stream", 'maxBodyLength': Infinity }) //maxContentLength: Infinity,
                
                await client.putFileContents(webdavFilename, fs.readFileSync(fileName), {
                    format: "binary", "Content-Type": "application/octet-stream", 'maxBodyLength': Infinity, onUploadProgress: progress => {
                        log.debug(`Uploaded ${progress.loaded} bytes of ${progress.total}`);
                    }
                })
                    .then(result => {
