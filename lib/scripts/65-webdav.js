...
async function command(options, log, callback) {
    if (options.username && options.pass && options.url && options.context.fileNames.length) {
        ...
        const agent = require('node:https').Agent({ rejectUnauthorized: options.signedCertificates });
                ...
                .then(contents => {
                    copyFiles(client, dir, fileNames, log, options.context.errors, err => {
                        ...
                        if (options.deleteOldBackup === true) {
                            const webdavDeleteAfter = options.webdavDeleteAfter === 0 ? options.deleteBackupAfter : options.webdavDeleteAfter;
