...
function command(options, log, callback) {
    setTimeout(() => {
        if (options.adapter) {
            ...
            const logFile = path.join(options.notification.bashDir, `${options.adapter.namespace}.log`);
            const tmpLog = path.join(options.backupDir, 'logs.txt');

            if (fs.existsSync(logFile) && fs.existsSync(tmpLog)) {
                const text = fs.readFileSync(tmpLog).toString();
                const data = fs.readFileSync(logFile, 'utf8');
                const backupLog = JSON.parse(data);
                const backupName = fileName.split('/').pop();

                try {
                    backupLog.unshift({ [backupName]: text });
                } catch (err) {
                    callback && callback(`Backup Logs could not be created: ${err}`);
                }

                if (backupLog && backupLog.length > options.notification.entriesNumber) {
                    backupLog.splice(options.notification.entriesNumber, backupLog.length - options.notification.entriesNumber);
                }
