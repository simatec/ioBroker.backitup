...
async function fileSizeCheck(fileName, log) {
    return new Promise(async (resolve) => {
        ...
        if (fs.existsSync(fileName)) {
            const stats = await fs_async.stat(fileName)
                .catch(err => log.warn('Filesize error: ' + err));

            fileSize = stats ? (Math.floor(stats.size / (1024 * 1024)) + ' MB') : 'n/A';
        }
        resolve(fileSize);
    });
}
            ...
            if (state) {
                ...
                if (historyListJSON !== undefined) {
                    try {
                        ...
                        log.error('history error: ' + e + ' Please reinstall backitup and run "iobroker fix"!!');
                    }
                }
