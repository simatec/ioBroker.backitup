...
function cleanFiles(client, dir, name, num, log, errors, callback) {
    try {
        client.list(dir, (err, result) => {
            ...
            if (result && result.entries && num) {
                ...
                if (result.length > num) {
                    // delete oldies files
                    result.sort((a, b) => {
                        const at = new Date(a.time?).getTime(); //todo
                        const bt = new Date(b.time?).getTime(); //todo
                        if (at > bt) return -1;
                        if (at < bt) return 1;
                        return 0;
                    });
