    ...
    if (options.ccuMulti) {
        ...
        for (let i = 0; i < options.ccuEvents.length; i++) {
            setTimeout(function () {
                ...
                startBackup(options, log, callback, num);
            }, 10000 * i, i);
        }
    } else {
        startBackup(options, log, callback, null);
    }
}

function startBackup(options, log, callback, num) {
                    ...
                    .on('close', () => {
                        ...
                        request.post({
                            ...
                            (err, response, body) => {
                                ...
                                log.debug(num)
                                log.debug(options.ccuEvents.length)
                                if (options.ccuMulti && num == options.ccuEvents.length) {
                                    callback && callback();
