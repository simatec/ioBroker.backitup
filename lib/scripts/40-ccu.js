    ...
    (err, response, body) => {
        ...
        request(`http://${options.host}/api/backup/version.cgi`, (err, response, body) => {
            ...
            request.get({
                ...
                const fileName = path.join(options.backupDir , `homematic${options.nameSuffix ? '_' + options.nameSuffix : ''}_${version}_${getDate()}_backupiobroker.tar.gz`);
                ...
                (err, response, body) => {
                    ...
                    options.context.types.push('ccu');
