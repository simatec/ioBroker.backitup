...
function command(options, log, callback) {
    ...
    options.adapter.getObjectView('system', 'instance', { startkey: 'system.adapter.sql.', endkey: 'system.adapter.sql.\u9999' }, async (err, instances) => {
        let resultInstances = [];
        if (!err && instances && instances.rows) {
            ...
            for (let i = 0; i < resultInstances.length; i++) {
                ...
                // Stop sql Instances
                options.adapter.getForeignObject(`system.adapter.${_id}`, function (err, obj) {
                    if (obj && obj != null && obj.common.enabled == true) {
                        options.adapter.setForeignState(`system.adapter.${_id}.alive`, false);
                    ...
                    if (err) {
                        ...
                        if (startAfterBackup) {
                            enabledInstances.forEach(enabledInstance => {
                                options.adapter.getForeignObject(`system.adapter.${enabledInstance}`, function (err, obj) {
                                    if (obj && obj != null && obj.common.enabled == false) {
                                        options.adapter.setForeignState(`system.adapter.${enabledInstance}.alive`, true);
