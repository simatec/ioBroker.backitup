        ...
        if (fs.existsSync(fileName)) {
            ...
            gDrive.createFolder(dir)
                .then(folderId => {
                    ...
                    try {
                        return gDrive.writeFile(folderId, onlyFileName, readStream, log);
                    } catch (err) {
                        err && log.error(`Google Drive: ${err}`);
...
function command(options, log, callback) {
    if (options.accessJson && options.context.fileNames.length) {
        ...
        let gDrive = new GoogleDrive(options.accessJson, options.newToken);
        ...
        copyFiles(gDrive, dir, fileNames, log, options.context.errors, err => {
            ...
            if (options.deleteOldBackup === true) {
                const googledriveDeleteAfter = options.googledriveDeleteAfter === 0 ? options.deleteBackupAfter : options.googledriveDeleteAfter;

                cleanFiles(gDrive, options, dir, options.context.types, googledriveDeleteAfter, log, options.context.errors, err => {
                    if (err) {
