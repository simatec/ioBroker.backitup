    ...
    return new Promise(async (resolve) => {
        ...
        if (available && available.status) {
            ...
            try {
                ...
                options.context.errors.grafana = JSON.stringify(err);
                log.error('Error on Grafana Dashoard Request: ' + err);
            }
            ...
            try {
                await Promise.all(dashBoards.map(async (dashBoard) => {
                    ...
                    try {
                        ...
                        options.context.errors.grafana = JSON.stringify(err);
                        log.error(`Error on Grafana Dashoard ${dashBoardData[0]} backup: ${err}`);
                    }
                ...
                options.context.errors.grafana = JSON.stringify(err);
                log.error('Error on Grafana Dashoard backup: ' + err);
            }
            // request finish
            resolve();
        } else {
            ...
            try {
                ...
                options.context.errors.grafana = JSON.stringify(err);
                log.error(`Grafana tmp directory "${tmpDir}" cannot deleted ... ${err}`);
    ...
    if (options && options.protocol && options.host && options.port && options.username && options.pass && options.apiKey) {
        ...
        if (!fs.existsSync(tmpDir)) {
            ...
            try {
                ...
                log.error(`Grafana tmp directory "${tmpDir}" cannot deleted`);
            }
