...
async function getData(options, log, dashboardDir, datasourceDir) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            await Promise.all(dashBoards.map(async (dashBoard) => {
                ...
                let json = dashBoardRequest.data.dashboard;

                json.id=null;
                const data = JSON.stringify(json);

                fs_async.writeFile(`${dashboardDir}/${dashBoardName}.json`, `{ "dashboard": ${data}, "overwrite": true }`);
            }));
        } catch (err) {
...
async function command(options, log, callback) {
    ...
    if (fs.existsSync(tmpDir) && fs.existsSync(datasourceDir) && fs.existsSync(dashboardDir)) {
        ...
        try {
            ...
            await getData(options, log, dashboardDir, datasourceDir);
