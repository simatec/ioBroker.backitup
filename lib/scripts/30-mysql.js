...
async function startBackup(options, log, callback) {
	return new Promise(async (resolve) => {
		...
		child_process.exec(`${options.exe ? options.exe : 'mysqldump'}  -u ${options.user} -p${options.pass} ${options.dbName} -h ${options.host} -P ${options.port}${options.mysqlQuick ? ' --quick' : ''}${options.skipSSL ? ' --skip-ssl' : ''} > ${fileNameMysql}`, { maxBuffer: 10 * 1024 * 1024 }, async (error, stdout, stderr) => {
			if (error) {
				let errLog = `${error}`;
				try {
