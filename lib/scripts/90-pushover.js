    ...
    setTimeout(() => {
        if (options.pushover.enabled &&
            ...
            if (!errors.length) {
                ...
                if (options.onedrive && options.onedrive.enabled && options.pushover.NoticeType === 'longPushoverNotice') {
                    messageText += _(', and stored in onedrive', options.pushover.systemLang);
                }
