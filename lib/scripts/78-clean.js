    ...
    try {
        ...
        names.forEach(name => {
            ...
            if (result && result.length && num) {
                ...
                let delOldBackups = num;

                if (name == 'influxDB' && options.influxDBMulti) delOldBackups = num * options.influxDBEvents.length;
                if (name == 'mysql' && options.mySqlMulti) delOldBackups = num * options.mySqlEvents.length;
                if (name == 'pgsql' && options.pgSqlMulti) delOldBackups = num * options.pgSqlEvents.length;
                if (name == 'homematic' && options.ccuMulti) delOldBackups = num * options.ccuEvents.length;
                log.debug('multi: ' + options.influxDBMulti)
                log.debug('events: ' + options.influxDBEvents.length)
                log.debug('Name: ' + name + ' / num: ' + delOldBackups);
                const files = [];

                if (result.length > delOldBackups) {
                    // delete oldies files
                    result.sort((a, b) => {
                    ...
                    for (let i = delOldBackups; i < result.length; i++) {
