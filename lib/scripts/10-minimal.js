...
function command(options, log, callback) {
    ...
    try {
        const fileName = path.join(options.backupDir , `minimal_${getDate()}${options.nameSuffix ? '_' + options.nameSuffix : ''}_backupiobroker.tar.gz`);
        ...
        cmd.on('close', code => {
            options.context.done.push('minimal');
            options.context.types.push('minimal');
            if (callback) {
        ...
        cmd.on('error', error => {
            options.context.errors.minimal = error;
