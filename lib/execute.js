...
function createBackupLog(config, adapter) {
    ...
    const tmpLog = path.join(config.notification.bashDir, 'logs.txt');
            ...
            if (func) {
                ...
                const log = {
                    debug: function (text) {
                        ...
                        writeIntoFile(fileName, `[DEBUG] [${name}] ${text}`);
                        adapter && adapter.setState('output.line', `[DEBUG] [${name}] - ${text}`, true);
                    },
                    warn: function (warning) {
                        ...
                        writeIntoFile(fileName, `[WARN] [${name}] ${warning}`);
                    ...
                    error: function (err) {
                        ...
                        writeIntoFile(fileName, `[ERROR] [${name}] ${err}`);
