    ...
    for (const name in scripts) {
        if (scripts.hasOwnProperty(name) && scripts[name] && (!config.afterBackup || scripts[name].afterBackup)) {
            ...
            switch (name) {
                ...
                case 'mysql':
                case 'redis':
                    if ((config.name === 'total' || config.name === 'minimal' && config.extensionsEnabled === true) && config[name] && config[name].enabled) {
                        func = scripts[name];
                        //options = config[name];
...
function startIOB() {
    ...
    const cmd = spawn(isWin ? 'start_b_IOB.bat' : 'bash', [isWin ? '' : 'start_b_IOB.sh'], {detached: true, cwd: __dirname, stdio: ['ignore', 'ignore', 'ignore']});
