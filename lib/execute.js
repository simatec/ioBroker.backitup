        ...
        if (scripts.hasOwnProperty(name) && scripts[name] && (!config.afterBackup || scripts[name].afterBackup)) {
            ...
            if (func) {
                ...
                for (const i in _options) {
                    if (_options[i].dropbox) _options[i].dropbox.accessToken = '****';
                    if (_options[i].cifs) _options[i].cifs.pass = '****';
                    if (_options[i].ftp) _options[i].ftp.pass = '****';
                    if (_options[i].googledrive) _options[i].googledrive.accessJson = '****';
                    if (_options[i].mysql) _options[i].mysql.pass = '****';
                    if (_options[i].ccu) _options[i].ccu.pass = '****';
                }
