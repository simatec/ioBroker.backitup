...
function createBackupLog(config, adapter) {
    ...
    try {
        const fileNames = JSON.parse(JSON.stringify(config.context.fileNames));
        ...
        adapter.log.error(`FileName error: ${err}`);
    }
    ...
    if (fs.existsSync(logFile)) {
        ...
        const backupName = fileName.split('/').pop();

        try {
            backupLog.unshift({ [backupName]: tmpLog });
        } catch (err) {
...
async function executeScripts(adapter, config, callback, scripts, code) {
    if (!scripts) {
        ...
        if (scripts.hasOwnProperty(name) && scripts[name] && (!config.afterBackup || scripts[name].afterBackup)) {
            ...
            if (func) {
                try {
                    ...
                    timerCleanFiles = setTimeout(async function () {
                ...
                const log = {
                    debug: async function (text) {
                        ...
                        tmpLog += `[DEBUG] [${name}] ${text}\n`;
                        await adapter.setStateAsync('output.line', `[DEBUG] [${name}] - ${text}`, true);
                    },
                    warn: async function (warning) {
                        ...
                        tmpLog += `[WARN] [${name}] ${warning}\n`;
                        await adapter.setStateAsync('output.line', `[WARN] [${name}] - ${warning}`, true);
                    },
                    error: async function (err) {
                        ...
                        await adapter.setStateAsync('output.line', `[ERROR] [${name}] - ${err}`, true);
                    }
                };

                try {
                    // generic Error handling for all synchron errors in backup scripts
                    func.command(options, log,async  (err, output, _code) => {
    ...
    await adapter.setStateAsync('output.line', `[EXIT] ${code || 0}`, true);
