...
function mount(options, log, callback) {
    ...
    if (options.mountType === 'CIFS') {
        ...
        log.debug(`cifs-mount command: "${options.sudo ? 'sudo mount' : 'mount'} -t cifs -o ${options.user ? 'username=' + options.user + ',password=****' : ''}${options.cifsDomain ? ',domain=' + options.cifsDomain : ''}${options.clientInodes ? ',noserverino' : ''},rw,uid=iobroker,gid=iobroker,file_mode=0777,dir_mode=0777,${options.smb} ${options.mount}${options.dir} ${backupDir}"`);
        child_process.exec(`${options.sudo ? 'sudo mount' : 'mount'} -t cifs -o ${options.user ? 'username=' + options.user + ',password=' + options.pass : ''}${options.cifsDomain ? ',domain=' + options.cifsDomain : ''}${options.clientInodes ? ',noserverino' : ''},rw,uid=iobroker,gid=iobroker,file_mode=0777,dir_mode=0777,${options.smb} ${options.mount}${dir} ${backupDir}`, (error, stdout, stderr) => {
            if (error) {
                log.debug('first mount attempt with smb option failed. try next mount attempt without smb option ...');
                log.debug(`cifs-mount command: "${options.sudo ? 'sudo mount' : 'mount'} -t cifs -o ${options.user ? 'username=' + options.user + ',password=****' : ''}${options.cifsDomain ? ',domain=' + options.cifsDomain : ''}${options.clientInodes ? ',noserverino' : ''},rw,uid=iobroker,gid=iobroker,file_mode=0777,dir_mode=0777 ${options.mount}${options.dir} ${backupDir}"`);
                child_process.exec(`${options.sudo ? 'sudo mount' : 'mount'} -t cifs -o ${options.user ? 'username=' + options.user + ',password=' + options.pass : ''}${options.cifsDomain ? ',domain=' + options.cifsDomain : ''}${options.clientInodes ? ',noserverino' : ''},rw,uid=iobroker,gid=iobroker,file_mode=0777,dir_mode=0777 ${options.mount}${dir} ${backupDir}`, (error, stdout, stderr) => {
                    if (error) {
                        let errLog = '' + error;
                        try {
