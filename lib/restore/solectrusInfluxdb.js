    ...
    return new Promise(async (resolve) => {
        try {
            ...
            if (object) {
                ...
                const ids = Object.keys(solectrusObjects);
                log.debug(`SOLECTRUS InfluxDB backup contains ${ids.length} object(s): ${ids.filter(id => id.startsWith('system.adapter.')).join(', ') || 'no instance object found'}`);
                ...
                for (const i in solectrusObjects) {
                    ...
                    if (_object) {
                        try {
                            ...
                            if (isInstanceObject) {
                                await adapter.extendForeignObjectAsync(solectrusObjects[i]._id, { native: solectrusObjects[i].native });
                                log.debug(`Restore SOLECTRUS InfluxDB config for: ${solectrusObjects[i]._id} (${Object.keys(solectrusObjects[i].native || {}).length} native keys)`);
                            } else {
                                await adapter.setForeignObjectAsync(solectrusObjects[i]._id, solectrusObjects[i]);
                            }
