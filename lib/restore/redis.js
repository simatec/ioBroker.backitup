                                ...
                                try {
                                    copyFile(path.join(tmpDir, file), path.join(pth, file), err => {
                                        if (err) {
                                            ...
                                            try {
                                                child_process.exec(`redis-cli shutdown nosave`, (error, stdout, stderr) => {
                                                    if (error) {
                                                        log.debug(`redis-cli restart error: "${error}"`);
                                                    } else {
                                                        log.debug(`redis-cli restart: "${stdout}"`);
                                                    }
                                                });
                                            } catch (err) {
                                                log.debug(`redis-cli restart error: "${err}"`);
                                            }
