...
function replayPgSql(options, fileNamePgsql, log, callback) {
 ...
 // psql -c "create database test1" postgresql://iobroker:iobroker@localhost:5432/
...
function restore(options, fileName, log, callback) {
    ...
    log.debug('Start pgsql Restore ...');

    let timer = setInterval(() => {
        if (fs.existsSync(fileNamePgsql))  {
            const stats = fs.statSync(fileNamePgsql);
            const fileSize = Math.floor(stats.size / (1024 * 1024));
            log.debug(`Extract pgsql Backupfile ${fileSize}MB so far...`);
        } else {
    ...
    try {
        targz.decompress({
            ...
            if (err) {
                log.error(err);
                if (callback) {
                    log.error('pgsql Restore not completed');
                    callback(err, stderr);
                    callback = null;
                }
            } else {
                replayPgSql(options, fileNamePgsql, log, err => {
                    ...
                    if (callback) {
                        ...
                        callback(null, 'pgsql restore done');
                        callback = null;
                    }
                });
            }
