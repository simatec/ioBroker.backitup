...
async function postData(options, callback, log, tmpDir) {
    return new Promise(async (resolve, reject) => {
        ...
        await Promise.all(dataSources.map(async (dataSource) => {
            ...
            await axios({
                ...
                log.debug(`cannot restore datasource "${dataSourceName}": ${JSON.stringify(err.response.data)}`)
            });
        }));
        ...
        await Promise.all(dashBoards.map(async (dashBoard) => {
            ...
            log.debug(`start restore for ${dashBoardPth}`);
            ...
            if (err) {
                ...
                if (callback) {
                    try {
                        ...
                        postData(options, callback, log, tmpDir);
                        /*
                        log.debug('Try deleting the Grafana tmp directory');
                        fse.removeSync(tmpDir);
                        if (!fs.existsSync(tmpDir)) {
