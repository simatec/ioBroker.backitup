...
function replayMySql(options, fileNameMysql, log, callback) {
    ...
    */
    try {
        child_process.exec(`mysql -u ${options.user} -p${options.pass} -h ${options.host} -P ${options.port} ${options.dbName} < ${fileNameMysql}`, (error, stdout, stderr) => {
            if (error) {
                let errLog = '' + error;
                errLog = errLog.replace(new RegExp(options.pass, 'g'), "****")
                return callback(errLog, stderr);
            }
        });
    } catch (e) {
        callback(e);
