...
function replayInfuxDB(options, tmpDir, log, callback) {
    ...
    if (options.dbversion == '1.x') {
        cmd = `${options.exe ? `"${options.exe}"` : 'influxd'} restore -portable -db ${options.dbName}${options.dbType == 'remote' ? ` -host ${options.host}:${options.port}` : ''} "${tmpDir}"`;
    } else if (options.dbversion == '2.x') {
        cmd = `${options.exe ? `"${options.exe}"` : 'influx'} restore --bucket ${options.dbName}${options.dbType == 'remote' ? ` --host ${options.protocol}://${options.host}:${options.port}` : ''} -t ${options.token} "${tmpDir}"`;
    }
