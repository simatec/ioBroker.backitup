...
function restore(options, fileName, log, adapter, callback) {
    ...
    const zigbeePth = path.join(options.path, `zigbee_${num[0]}`).replace(/\\/g, '/');
    ...
    if (fs.existsSync(zigbeePth)) {
        try {
            fse.emptyDirSync(zigbeePth);
            if (!fs.readdirSync(zigbeePth).length) {
    ...
    adapter.getForeignObject(`system.adapter.zigbee.${num[0]}`, (err, obj) => {
