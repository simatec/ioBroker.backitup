...
class GoogleDrive {
            ...
            this._getFileOrFolderId([dir], folderId, (err, _folderId) => {
                if (!_folderId) {
                        ...
                        drive.files.create({
                            ...
                            if (err) {
                                ...
                                if (!parts.length || !__folderId) {
                                    ...
                                    setImmediate(_createFolder, parts, __folderId, callback);
                                }
                            }
                        }));
                } else {
                    if (!parts.length || !_folderId) {
                        ...
                        setImmediate(() => this._createFolder(parts, _folderId, callback));
                    }
                }
            });
        }
    ...
    deleteFile(folderId, fileName) {
        return new Promise((resolve, reject) => {
            this.getFileOrFolderId(fileName, folderId)
                .then(fileId => {
                    this._getDrive()
                        .then(drive => drive.files.delete(fileId, (err, file) => {
                            if (err) {
                                // Handle error
                                reject(err);
                            } else {
            ...
            .then(drive => drive.files.list({
                ...
                if (err) {
                    ...
                    if (!parts.length || !folderId) {
                        ...
                        setImmediate(() => this._getFileOrFolderId(parts, folderId, callback));
                    }
                }
            }));
    }
            ...
            .then(drive => drive.files.list({
                ...
                if (err) {
                    ...
                    if (res.nextPageToken) {
                        setTimeout(() => this._listFilesInFolder(folderId, cb, res.nextPageToken, _list), 50);
                    } else {
                        cb(null, _list);
                    }
                }
