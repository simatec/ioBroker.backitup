        ...
        if (adapterConf && adapterConf.common && adapterConf.common.host) {
            ...
            if (_diskFree && _diskFree.val) {
                ...
                const sysCheck = {
                    ...
                    ready: options.config.cifsEnabled || _diskFree.val > 512 ? true : false
                };

                switch (sysCheck.diskState) {
                    case 'ok':
                        log.debug(`On the host "${adapterConf.common.host}" are currently ${_diskFree.val} MB free space available!`);
                        break;
                    case 'warn':
