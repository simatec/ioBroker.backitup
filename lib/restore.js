...
function startIOB(bashDir) {
    ...
    if (fs.existsSync('/opt/scripts/.docker_config/.thisisdocker')) {
        timeFinish = 3000;
        logWebIF += '[iobroker] [Debug] Docker Container restart now\n';
    }
...
function restoreIF(currentTheme, bashDir) {
    ...
    app.get('/backitup-restore.html', function (req, res) {
        ...
        let content = (
            ...
            '               <input class="button" style="display: ' + (startFinish == '[Restart]' ? 'initial;' : 'none;') + '" type="button" value="Backup Menu" onClick="javascript:history.back()">' +
            '           </div>' +
            '       </div>' +
            '   </div>' +
            '</body>'
