...
function httpServer() {
	...
	const app = express()
		...
		app.get('/', function (req, res) {
			...
			res.send('<html><head>' + '<meta http-equiv="refresh" content="5"/>' + '</head><body>' + 'Log Backitup Restore:<br><br>' + data + '</body></html>');
		})
		app.listen(8091)
    /*
	setInterval(function () {
		...
		const srv = http.createServer((req, res) => {
			res.write('<html><head>' + '<meta http-equiv="refresh" content="10"/>' + '</head><body>' + '</body></html>');
			res.write('Log Backitup Restore:<br><br>' + data);
			res.end();
