...
function restore(adapter, options, storageType, fileName, currentTheme, bashDir, log, callback) {
            ...
            if (!err && fs.existsSync(toSaveName)) {
                ...
                if (_module.isStop) {
                    if (backupType == 'iobroker' && fs.existsSync(bashDir)) {
                        ...
                        tools.copyFile(path.join(__dirname, 'restore.js'), path.join(bashDir, 'restore.js'), callback);

                        if (!fs.existsSync(restoreDir)) {
                            fs.mkdirSync(restoreDir);
                        }

                        tools.copyFile(path.join(restoreSource, `${backupType}.js`), path.join(restoreDir, `${backupType}.js`), callback);
                    }
...
function restoreIF(currentTheme, bashDir) {
    const http = require('http');
    const https = require('https');
    const express = require('express');
    const app = express();

    const privateKey = fs.readFileSync(bashDir + '/iob.key', 'utf8');
    const certificate = fs.readFileSync(bashDir + '/iob.crt', 'utf8');
    ...
    app.get('/backitup-restore.html', function (req, res) {
        ...
        let content = (
            ...
            '               <input class="button" style="display: ' + (startFinish == '[Finish]' ? 'initial;' : 'none;') + '" type="button" value="Backup Menu" onClick="javascript:history.back()">' +
            '           </div>' +
            '       </div>' +
            '   </div>' +
            '</body>'
...
if (typeof module !== "undefined" && module.parent) {
    ...
    if (fs.existsSync(__dirname + '/restore.json')) {
        ...
        startFinish = '[Restore]';
        restoreIF(config.theme, config.bashDir);
        ...
        restore(null, config, null, null, null, null, log, () => startIOB(config.bashDir));
    } else {
