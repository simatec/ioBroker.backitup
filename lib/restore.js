...
function restoreIF() {
    app.get('/backitup-restore', function (req, res) {
        let htmlData = '';
		const head = ('<style>body {font-family: monospace; font-size: 14px;}</style>');
        if (!logWebIF == '') {
    ...
    if (fs.existsSync(__dirname + '/restore.json')) {
        ...
        const log = {
            ...
            exit: function (exitCode) {
                ...
                if (exitCode == 0) {
                	logWebIF = (logWebIF + `<font color="2EFE2E">[EXIT] ${exitCode} **** Restore completed successfully!! ****</font><br>`);
				} else {
					logWebIF = (logWebIF + `<font color="red">[EXIT] ${exitCode} **** Restore was canceled!! ****</font><br>`);
				}
            }
        };
