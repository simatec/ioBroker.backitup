...
function restore(adapter, options, storageType, fileName, host, log, callback) {
    ...
    if (adapter) {
        ...
        getFile(options, storageType, fileName, toSaveName, log, err => {
            if (!err && fs.existsSync(toSaveName)) {
                ...
                if (backupType == 'redis') {
                    fs.writeFileSync(__dirname + '/.redis.info', 'restart redis after Restore');
                    log.debug('Redis restarted');
                }
                ...
                if (_module.isStop) {
                    ...
                    _module.restore(config, toSaveName, host, log, adapter, (err, exitCode) => {
                        log.exit(exitCode);
        ...
        try {
            ...
            _module.restore(config, config.fileName, host, log, (err, exitCode) => {
                log.exit(exitCode);
                callback({ error: err, exitCode })
            });
        } catch (e) {
...
function restoreIF() {
    ...
    app.get('/backitup-restore.html', function (req, res) {
        ...
        let content = (
            ...
            '       </div>' +
            '       <div class="row">' +
            '           <a href="' + iobrokerHost + '" class="status" style="color: lightskyblue; display: ' + (startFinish ? 'initial;' : 'none;') + '">Please open your ioBroker here</a>' +
