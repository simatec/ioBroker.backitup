...
function startIOB() {
    ...
    child_process.exec(isWin ? 'startIOB.bat' : 'bash startIOB.sh', (error, stdout, stderr) => {
        ...
        setTimeout(() => startFinish = true, 10 * 1000);
        setTimeout(() => process.exit(), 30 * 1000);
    });
}

function restore(adapter, options, storageType, currentTheme, fileName, log, callback) {
...
function restoreIF(currentTheme) {
    ...
    app.get('/backitup-restore.html', function (req, res) {
        const refresh = startFinish ? false : true;
        const darkDesign = currentTheme == 'dark-react' || currentTheme == 'dark-blue' ? true : false;
        ...
        */
        ...
        let style = '<style>';
        style += 'body { background-color: ' + (darkDesign ? '#3b3b3b' : '#fff') +' }';
        style += '.box { overflow-y: auto; border-radius: 4px; ' + (darkDesign ? 'color: #fff; background: #1f1f1f' : 'color: #1f1f1f; background: #fff') + ' font-family: courier, monospace; font-size: 14px; resize: none; width: 100%; margin-bottom: 0;padding: 0.7rem; }';
        ...
        style += '.container { background-color: ' + (darkDesign ? '#3b3b3b' : '#fff') +' }';
        style += '.status { font-size: 1.5rem; padding-left: 20px; font-family: system-ui; margin-block-start: 0.1em; margin-block-end: 0.1em;font-weight: bold;}';
        style += '</style>';
        ...
        let content = (
            ...
            '       </div>' +
            '       <div class="row">' +
            //'           <a href="' + hostIP + '" class="status" style="color: lightskyblue; display: ' + (startFinish ? 'initial;' : 'none;') + '">Please open your ioBroker here</a>' +
            '           <h6 class="status" style="color: ' + (darkDesign ? '#fff' : '#3b3b3b') + '; display: ' + (startFinish ? 'initial;' : 'none;') + '">Restore and Restart completed successfully!!</h6>' +
            '           <h6 class="status" style="color: ' + (statusColor ? statusColor : '#fff') + '; display: ' + (startFinish ? 'none;' : 'initial;') + '">' + (restoreStatus ? restoreStatus : 'Restore is started ...') + '</h6>' +
            '       </div>' +
            '       <div class="row" style="padding: 20px;">' +
            '           <div class="col s12">' +
            '               <textarea class="box" readonly rows="30">' + logWebIF +
            '               </textarea>' +
            '           </div>' +
            '       </div>' +
            '   </div>' +
        ...
        const log = {
            ...
            exit: function (exitCode) {
                ...
                if (exitCode == 0 || exitCode == 'mysql restore done' || exitCode == 'redis restore done' || exitCode == 'historyDB restore done' || exitCode == 'zigbee database restore done' || exitCode == 'yahka database restore done' || exitCode == 'jarvis database restore done' || exitCode == 'influxDB restore done' || exitCode == 'postgresql restore done' || exitCode == 'Grafana restore done' || exitCode == 'javascript restore done') {
                    ...
                    statusColor = 'red';
                    restoreStatus = 'Restore was canceled!! Starting iobroker... Please wait!';
                }
            }
        };
