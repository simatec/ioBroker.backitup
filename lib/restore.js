...
function restoreIF() {
    ...
    app.listen(8091) //8091 port what this port? 8081 is default iobroker.admin port
}
    ...
    if (fs.existsSync(__dirname + '/restore.json')) {
        ...
        const log = {
            ...
            exit: function (exitCode) {
                ...
                if (exitCode == 0 || 'mysql restore done' ||'postgresql restore done'|| 'redis restore done' || 'historyDB restore done' || 'zigbee database restore done') {
