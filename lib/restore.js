        ...
        getFile(options, storageType, fileName, toSaveName, log, err => {
            if (!err && fs.existsSync(toSaveName)) {
                ...
                if (_module.isStop) {
                    ...
                    const log = {
                        debug: function (text) {
                            ...
                            lines.forEach(line => {
                                ...
                                //adapter && adapter.setState('output.line', '[DEBUG] [' + backupType + '] - ' + line);
                            });
                    ...
                    //restoreIF();
                    _module.restore(config, toSaveName, log, (err, exitCode) => {
                        log.exit(exitCode);
    ...
    if (fs.existsSync(__dirname + '/restore.json')) {
        ...
        const log = {
            ...
            exit: function (exitCode) {
                ...
                if (exitCode == 0 || exitCode == 'mysql restore done' || exitCode == 'redis restore done' || exitCode == 'historyDB restore done' || exitCode == 'zigbee database restore done' || exitCode == 'jarvis database restore done' || exitCode == 'influxDB restore done' || exitCode == 'postgresql restore done' || exitCode == 'Grafana restore done' || exitCode == 'javascript restore done') {
