    ...
    if (fs.existsSync(__dirname + '/restore.json')) {
        ...
        const log = {
            ...
            exit: function (exitCode) {
                ...
                if (exitCode == 0 || exitCode == 'mysql restore done' || exitCode == 'redis restore done' || exitCode == 'historyDB restore done' || exitCode == 'zigbee database restore done' || exitCode == 'jarvis database restore done' || exitCode == 'influxDB restore done' || exitCode == 'postgresql restore done' || exitCode == 'Grafana restore done') {
