            ...
            if (!err && fs.existsSync(toSaveName)) {
                ...
                if (_module.isStop) {
                    if (backupType === 'iobroker' && fs.existsSync(bashDir)) {
                        ...
                        tools.copyFile(fs.readFileSync(path.join(__dirname, '..', 'admin', 'favicon.ico'), path.join(bashDir, 'favicon.ico')));
...
function restoreIF(currentTheme, currentProtocol, bashDir) {
    ...
    app.get('/backitup-restore.html', function (req, res) {
        res.send(fs.readFileSync(path.join(bashDir, 'backitup-restore.html')));
    });
