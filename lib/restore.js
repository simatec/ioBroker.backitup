                    ...
                    return callback && callback({error: ''});
                        ...
                        debug: function (text) {
                            ...
                            lines.forEach(line => {
                                ...
                                line && adapter && adapter.log.debug(`[${backupType}] ${line}`);
                            });
                        ...
                        error: function (err) {
                            ...
                            lines.forEach(line => {
                                line = line.replace(/\r/g, ' ').trim();
                                line && adapter && adapter.log.error(`[${backupType}] ${line}`);
                            });
    ...
    if (fs.existsSync(__dirname + '/restore.json')) {
        ...
        const log = {
            ...
            exit: function (exitCode) {
                ...
                if (exitCode == 0 || exitCode == 'mysql restore done' || exitCode == 'influx restore done' || exitCode == 'redis restore done' || exitCode == 'historyDB restore done' || exitCode == 'zigbee database restore done') {
