        ...
        const log = {
            debug: function (text) {
                ...
                lines.forEach(line => {
                    ...
                    line && writeIntoFile(logWebIF, `<font color="F2F2F2">[DEBUG] [${config.name}] ${line}</font><br>`);
                });
            },
            error: function (err) {
                const lines = err.toString().split('\n');
                lines.forEach(line => {
                    ...
                    line && writeIntoFile(logWebIF, `<font color="red">[ERROR] [${config.name}] ${line}</font><br>`);
                });
            },
            exit: function (exitCode) {
                writeIntoFile(logName, `[EXIT] ${exitCode}`);
                writeIntoFile(logWebIF, `<font color="F2F2F2">[EXIT] ${exitCode}</font><br>`);
            }
        };
