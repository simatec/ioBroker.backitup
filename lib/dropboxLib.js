...
class dropbox {
    ...
    sessionUpload(dbx, fileName, dir, log) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (fileSize) {
                    ...
                    const append = (sessionId, start, end) => {
                        if (start === fileSize) {
                            ...
                            resolve('done');
                            return this.sessionFinish(sessionId, dbx, log, dir, onlyFileName, fileSize);
                        }

                        if (end > fileSize) {
                            end = fileSize - 1;
                            log.debug(`${Math.round((start / fileSize) * 100)}% uploaded from ${onlyFileName}`);
                            return this.sessionAppend(sessionId, dbx, getNextChunkStream, log, start, fileSize - 1, () => {
                                log.debug(`${Math.round((end / fileSize) * 100)}% uploaded from ${onlyFileName}`);
                                resolve('done');
                                return this.sessionFinish(sessionId, dbx, log, dir, onlyFileName, fileSize);
                            })
                        ...
                        log.debug(`${Math.round((start / fileSize) * 100)}% uploaded from ${onlyFileName}`);
                        this.sessionAppend(sessionId, dbx, getNextChunkStream, log, start, end, () => {
                            append(sessionId, end + 1, end + chunkLength)
                        });
                    }

                    this.sessionStart(dbx, log, (sessionId) => {
                ...
                reject(`Error Session Upload: ${err}`);
            }
        });
    }

    sessionStart(dbx, log, cb) {
        dbx({
    ...
    sessionAppend(sessionId, dbx, getNextChunkStream, log, start, end, cb) {
        dbx({
    ...
    sessionFinish(sessionId, dbx, log, dir, onlyFileName, fileSize) {
