...
module.exports = function (adapter, config, log, callback) {
    const files = {};
    ...
    if (fs.existsSync(backupDir) && adapter.config.restoreSource === 'Local') {
        ...
        }).filter(file => file.name.match(/^\d\d\d\d_\d\d_\d\d-\d\d_\d\d_\d\d_backupiobroker\.tar\.gz$/) || creators.indexOf(file.name.split('_')[0]) !== -1);
        files.local = {};
        fff.forEach(file => {
        ...
        if (config.hasOwnProperty(type)) {
            for (const attr in config[type]) {
                if (config[type].hasOwnProperty(attr) &&
                    ...
                    counter++;
                    storages[attr].list(config[type][attr], creators, log, (err, result, type) => {
                        err && log.error(err);
                        if (result) {
                            if (type === 'cifs') {
